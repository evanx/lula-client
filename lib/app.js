...
const buildServices = app => ({
  login: require('./login')(app),
  poll: require('./poll')(app),
        ...
        message(response) {
          ...
          try {
            if (response.type === 'err') {
              ...
              reject({ message: payload.err.message })
            }
...
app.start = async ({ fetch, secret }) => {
  ...
  while (true) {
    if (app.ws.readyState === WebSocket.OPEN) {
      const pollRes = await app.services.poll()
      const publishRes = await app.services.publish()
    } else if (app.ws.readyState === WebSocket.CLOSED) {
      monitor.debug('ws CLOSED')
    }
